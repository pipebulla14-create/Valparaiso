...
from rasterio.mask import mask  # <--- ESTA ES LA IMPORTACIÓN QUE FALTABA
...
def generar_leyenda_severidad_html():
    ...
    <div style="position: fixed; bottom: 50px; left: 20px; width: 250px; z-index:9999; background-color: rgba(255, 255, 255, 0.85); backdrop-filter: blur(6px); padding: 15px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.4);">
        ...
        <div style="display:flex; align-items:center;"><div style="background-color: rgba(255,255,0,0.9); width: 20px; height: 20px; margin-right: 10px; border-radius: 3px;"></div> Baja (0.10 - 0.27)</div>
        <div style="display:flex; align-items:center;"><div style="background-color: rgba(255,153,0,0.9); width: 20px; height: 20px; margin-right: 10px; border-radius: 3px;"></div> Moderada (0.27 - 0.44)</div>
        <div style="display:flex; align-items:center;"><div style="background-color: rgba(255,0,0,0.9); width: 20px; height: 20px; margin-right: 10px; border-radius: 3px;"></div> Alta (≥ 0.44)</div>
    </div>"""

def procesar_raster_color(ruta, tipo):
    with rasterio.open(ruta) as src:
        # Reproyección simple a 4326
        transform, width, height = calculate_default_transform(src.crs, "EPSG:4326", src.width, src.height, *src.bounds)
        banda = np.zeros((1, height, width), dtype=np.float32)
        reproject(source=rasterio.band(src, 1), destination=banda[0], src_transform=src.transform, src_crs=src.crs, dst_transform=transform, dst_crs="EPSG:4326", resampling=Resampling.nearest)
        
        rgba = np.zeros((height, width, 4), dtype=np.uint8)
        valido = (banda[0] > 0)
        if tipo == "severidad":
            rgba[(banda[0] >= 0.10) & (banda[0] < 0.27) & valido] = [255, 255, 0, 200]
            rgba[(banda[0] >= 0.27) & (banda[0] < 0.44) & valido] = [255, 153, 0, 200]
            rgba[(banda[0] >= 0.44) & valido] = [255, 0, 0, 200]
    ...
    if show_pobladas and show_severidad and 'gdf_pob' in locals():
        with rasterio.open(DATA / "VALPO_severidad_dNBR_solo_quemado.tif") as src:
            ...
            if gdf_pob_proj.intersects(box(*src.bounds)).any():
                ...
                c3.metric("🏘️ Área Poblada Quemada", "N/A")
